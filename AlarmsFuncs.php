...
//******************************************************************************
		...
		if(create_connection($connection))
		{
			...
			if($userInfo != null)
			{
				...
				if(count($userAlarms) > 0)
				{
					...
					$mailbody = file_get_contents("/var/www/html/ewatcher-users/EmailTemplate.html");
					$fecha = date("d/m/T H:i");
					$mailbody = str_replace("[DATE_INFO]", "Informe generado en ".$fecha,$mailbody);
	...
	function formatMailBody($userAlarms)
	{
		...
		$result .= "<table border=\"1\" style=\"background-color:grey;\">\n";
		$result .= "<tr style=\"background-color:#FFE0C0;\"><th>NODO</th><th>Alarma</th></tr>\n";
		for($r=0; $r<count($userAlarms); $r++)
		{
			$result .= "<tr style=\"background-color:#FFFFC0;\">";
			$result .= "<td>".$userAlarms[$r][2]."</td><td>".$userAlarms[$r][1]."</td>";
			$result .= "</tr>";
		...
		for($r=0; $r < count($rad_power_alarms); $r++)
		{
			...
			if($rad_power_alarms[$r][0] == $userid)
			{
				...
				if (($rad_InputInfo != null) && ($power_InputInfo != null))
				{
					...
					$power_nodeid = $power_InputInfo[4];
					
					if (($rad_value > $RadThreshold) && ($power_value == 0))
					{
						$alarmText = "Potencia baja: MEDIDA: ".$power_InputInfo[3]." (".$power_InputInfo[2].") - VALOR: ".$power_value."; Radiación global: ".$rad_value;
